...
{
    public partial class EditVideoSessionFile : BasePage
    {
            ...
            if (!string.IsNullOrEmpty(SessionFileID))
            {
                ...
                if (file != null && file.Status == 1)
                {
                   // string mp3FilePath = string.Format("{0}{1}", AppConfig.GetInstance().AudioServerPath, file.Name);
                    string txtFilePath = string.Format("{0}{1}", AppConfig.GetInstance().VecSysServerPath, file.Name.ToLower().Replace("mp4", "txt"));
                    string xmlFilePath = string.Format("{0}{1}", AppConfig.GetInstance().VecSysServerPath, file.Name.ToLower().Replace("mp4", "trans.xml"));
                    ...
                    if (File.Exists(xmlFilePath))
                    {
                        ...
                        else
                        {
                            ...
                            elm1.Value = "";
                            MP3FilePath.Value = string.Format("{0}://{1}:{2}/sessionfiles/", Request.Url.Scheme, Request.Url.Host, Request.Url.Port) + file.Name;
                        }
                    }
                    else
                    {
